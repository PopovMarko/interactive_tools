...
func New(cfg *pomodoro.IntervalConfig) (*App, error) {
	ctx, cancel := context.WithCancel(context.Background())

	quiter := termdash.KeyboardSubscriber(func(k *terminalapi.Keyboard) {
		if k.Key == 'q' || k.Key == 'Q' {
			cancel()
		}
	})
	...
	b, err := newButtonSet(ctx, cfg, w, redrawCh, errorCh)
	if err != nil {
		return nil, err
	}

	container, err := newGrid(b, w, term)
	if err != nil {
		return nil, err
	}

	controller, err := termdash.NewController(term, container, quiter)
	if err != nil {
		return nil, err
	}
	return &App{
